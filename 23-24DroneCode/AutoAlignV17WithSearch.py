...
xy_error_threshold_stage1 = 1.25  # the threshold under which to begin dropping altitude while aligning
...
def final_targeting_stage(marker_ids, marker_centers, all_distances, shot_marker, final_targeting_until_time,
    ...
    if marker_to_engage != -1 and all(
        ...
        if abs(vehicle.velocity[0]) < 0.01 and total_error_average <= max_engagement_angle and shot_marker == 0 and (
                (ALIGN_VEHICLE_TO_MARKER_ALT + 0.2) > vehicle.location.global_relative_frame.alt > (
                ALIGN_VEHICLE_TO_MARKER_ALT - 0.2)):
...
def capture_frames(queue):
    ...
    zed.set_camera_settings(sl.VIDEO_SETTINGS.EXPOSURE, 1.25)  # -1 is default i think
