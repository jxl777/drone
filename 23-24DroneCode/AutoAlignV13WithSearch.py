...
image_data = {'Altitude': 3.1,  # meters
              ...
              'BatteryVoltage': 14.7,
              'ElapsedTime': '10:10:001'}  # Battery Voltage
...
def run_path_generation(vehicle, frame_width_meters, frame_height_meters):
    ...
    if is_test is True:
        vehicle.home_location = LocationGlobal(vehicle.location.global_frame.lat, vehicle.location.global_frame.lon,
...
def process_hud(hud_data_queue=None, hudplot_queue=None):
	data = hud_data_queue.get()
	
	hudplot_queue.put(all_data)
	


def process_frames(queue=None, hud_data_queue=None, hudplot_queue=None):
    global image_data
    global GPS_history
    global zed
    global roll
    ...
    while True:
        ...
        if not queue.empty():
            ...
            if has_path_been_generated is True:
                ...
                combined_image = get_combined_image(graph_image, frame_with_overlay)  # get the combined image
...
if __name__ == '__main__':
    ...
    hud_data_queue = hudplot_queue = multiprocessing.Queue(maxsize=2)
