...
search_speed = 4.0  # m/s speed to search for marker in waypoint stage
align_speed = 0.65  # m/s speed to align vehicle to marker
max_single_id_search_time = 15  # time in seconds to search for and identified marker before giving up
...
xy_error_threshold_stage1 = 1.7  # the threshold under which to begin dropping altitude while aligning
...
marker_to_engage = -1
ALLOWABLE_MARKER_MISSING_TIME = 4  # max time marker is allowed to be out of frame before returning to last known location
...
image_export_folder_path = "/home/enorda/Desktop/Auto12/VideoLogs/"
...
def final_targeting_stage(marker_ids, marker_centers, all_distances, shot_marker, final_targeting_until_time,
    ...
    if marker_to_engage != -1 and all(
        ...
        if abs(vehicle.velocity[0]) < 0.01 and total_error_average <= max_engagement_angle and shot_marker == 0 and ((ALIGN_VEHICLE_TO_MARKER_ALT+0.2) > vehicle.location.global_relative_frame.alt > (ALIGN_VEHICLE_TO_MARKER_ALT-0.2)) :
...
def capture_frames(queue):
    ...
    zed.set_camera_settings(sl.VIDEO_SETTINGS.EXPOSURE, 2)  # -1 is default i think
            ...
            try:
                if hist_of_shots is not None and last_hist_of_shots != hist_of_shots:
                    ...
                    for shot in range(len(hist_of_shots)):
                        plt.scatter(hist_of_shots[shot][1], hist_of_shots[shot][2], color='blue', marker='x', s=100)  # Plot the point
                        plt.text(hist_of_shots[shot][1], hist_of_shots[shot][2], f'ID:{hist_of_shots[shot][0]}\nLat:{hist_of_shots[shot][1]}\nLon:{hist_of_shots[shot][2]}', fontsize=8, ha='right', va='bottom')  # Annotate the point with label
...
def save_image_from_queue(image_queue, folder_path):
    ...
    if not os.path.exists(folder_path):
        ...
        # If folder exists, find the next available folder name
        # Get a list of existing folders in the parent directory
        existing_folders = [folder for folder in os.listdir(os.path.dirname(folder_path)) if
                            os.path.isdir(os.path.join(os.path.dirname(folder_path), folder))]

        # Extract the numbers from the folder names and find the maximum
        existing_numbers = [int(folder_name.split("video_data_log")[-1]) for folder_name in existing_folders if
                            folder_name.startswith("foldername")]
        next_number = max(existing_numbers, default=0) + 1

        folder_path = os.path.join(os.path.dirname(folder_path), f"video_data_log{next_number}")
    ...
    while True:
        try:
            if not hudplot_queue.empty():
                ...
                # Generate filename (you may want to make this more sophisticated)
                filename = os.path.join(folder_path, f"video_log_image_{frame_number}.jpg")
            ...
            # Queue is empty, continue waiting for new images
            pass
    ...
    while True:
        ...
        if not queue.empty():
            ...
            if has_path_been_generated is True:
                ...
                # skewed_grid_points = np.loadtxt('generated_search_pattern_waypoints.csv', delimiter=',', skiprows=1)
                if is_test is True:
                    GPS_history = [[vehicle.location.global_frame.lat + l, vehicle.location.global_frame.lon + o]]
                else:
                ...
                if marker_to_engage == -1:
                    if search_waypoints is not None:
                        ...
                        distance_to_waypoint = waypoint_distance(vehicle.location.global_relative_frame,
                                                                 current_waypoint)
                        if distance_to_waypoint < 0.75:  # has the vehicle reached the current waypoint target
                        ...
                        if (search_waypoint_index > final_search_waypoint_index) or (
                                number_of_markers_engaged >= number_of_markers_to_engage) and finished_execution is False:  # if the vehicle has reached the end of the search or has shot all vehicles
                ...
                if shot_marker == 1:
                    if shot_hist_updated is False:
                        ...
                        if marker_shot_history is None:
                            marker_shot_history = [[marker_to_engage,vehicle.location.global_frame.lat, vehicle.location.global_frame.lon]]
                        else:
                            marker_shot_history.append([marker_to_engage,vehicle.location.global_frame.lat, vehicle.location.global_frame.lon])
...
if __name__ == '__main__':
    ...
    image_save_process = multiprocessing.Process(target=save_image_from_queue,
                                                 args=(image_queue, image_export_folder_path))
