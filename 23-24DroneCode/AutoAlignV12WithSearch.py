    ...
    while True:
        ...
        if not queue.empty():
            ...
            if has_path_been_generated is True:
                ...
                #this_frame = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
                #this_frame = add_timestamp_overlay(this_frame)
                
                # Write frame to video
                
                #frame_rgb = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
                #video_writer.write(frame_rgb)
                cv2.imwrite(f'/home/enorda/Desktop/Auto12/images/OutputImage{frame_num}.jpg', frame)
                frame_num += 1
            ...
            if run_initial_search is True:
                ...
                if has_path_been_generated is False:
                    ...
                    #video_name = "uav_video_data_log_1"

                    #video_writer = cv2.VideoWriter(video_name, cv2.VideoWriter_fourcc(*'MJPG'), float(frame_rate),(frame.shape[1], frame.shape[0]), True)
                    
                    # Define the codec and create VideoWriter object
                    #fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                    #fourCC = cv2.VideoWriter_fourcc('X','V','I','D')
                    
                    #video_writer = cv2.VideoWriter('/home/enorda/Desktop/Auto12/1outputVideo.avi',fourCC , 20.0, (frame.shape[1], frame.shape[0]))
                    
                    #video_writer = imageio.get_writer("output.mp4", fps=9, codec='libx264')
