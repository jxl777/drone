...
def find_last_video_file(header):
    files = [f for f in os.listdir() if f.startswith(header) and f.endswith('.mp4')]
    if files:
...
def generate_video_name(header):
    last_file = find_last_video_file(header)
    video_counter = 1
    if last_file:
        file_number = int(last_file.split('_')[-1].split('.')[0])
        video_counter = file_number + 1
    return f"{header}{video_counter}.mp4"
        ...
        if not queue.empty():
            ...
            if run_initial_search is True:
                ...
                if has_path_been_generated is False:
                    ...
                    video_name = generate_video_name(video_name_header)
                    print(f"videoname: {video_name}")
                    video_name = "uav_data_video.mp4"

                    video_writer = cv2.VideoWriter(video_name, cv2.VideoWriter_fourcc(*'mp4v'), frame_rate,
                    ...
                    search_waypoints, point1, point2, landing_zone_waypoint = run_path_generation(vehicle, camera_frame_width,
                ...
                if marker_to_engage == -1:
                    if search_waypoints is not None:
                        ...
                        if (search_waypoint_index >= final_search_waypoint_index) or (number_of_markers_engaged >= number_of_markers_to_engage): # if the vehicle has reached the end of the search or has shot all vehicles
                            if number_of_markers_engaged >= number_of_markers_to_engage: # shot all markers
                                ...
                                print(f"Final Search Waypoint Reached without engaging all: {number_of_markers_to_engage} Markers, Total Engaged: {number_of_markers_engaged}")
                            #vehicle.mode = VehicleMode("RTL")
                            home_altitude = vehicle.home_location.alt
                            target_altitude = home_altitude + ALTITUDE
                            vehicle.simple_goto(LocationGlobal(landing_zone_waypoint[0], landing_zone_waypoint[1],target_altitude), groundspeed=search_speed) # go to the end of the field landing location
                            landing_zone_waypoint_loc = Location(*landing_zone_waypoint)
                            while True:
                                dist_to_waypoint = waypoint_distance(vehicle.location.global_frame,landing_zone_waypoint_loc)
