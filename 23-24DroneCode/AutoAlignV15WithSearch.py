...
def final_targeting_stage(marker_ids, marker_centers, all_distances, shot_marker, final_targeting_until_time, actuator_latency):
    global zed
    global roll
    global pitch
...
def capture_frames(queue):
    ...
    zed.set_camera_settings(sl.VIDEO_SETTINGS.EXPOSURE, 1)  # -1 is default i think
...
def find_next_folder_name(folder_path):
    ...
    existing_folders = [folder for folder in os.listdir(os.path.dirname(folder_path)) if os.path.isdir(os.path.join(os.path.dirname(folder_path), folder))]

    # Extract the numbers from the folder names and find the maximum
    existing_numbers = [int(folder_name.split("foldername")[-1]) for folder_name in existing_folders if folder_name.startswith("foldername")]
...
def save_image_from_queue(image_queue, folder_path):
    ...
    if not os.path.exists(folder_path):
        ...
        # Extract the numbers from the folder names and find the maximum
        existing_numbers = [int(folder_name.split("video_data_log")[-1]) for folder_name in existing_folders if folder_name.startswith("foldername")]
        next_number = max(existing_numbers, default=0) + 1
...
def process_frames(queue, hud_data_queue, hudplot_queue):
    ...
    while True:
        ...
        if not queue.empty():
            ...
            if (elapsed_marker_engage_time > max_single_id_search_time) and marker_to_engage != -1:  # if we have been searching too long without success
...
if __name__ == '__main__':
    ...
    image_save_process = multiprocessing.Process(target=save_image_from_queue, args=(image_queue, image_export_folder_path))                   
